...
class AbletonLinkTest < Minitest::Test
  ...
  def test_time_until_beat_within_bar
    ...
    assert_in_delta @link.time_until_beat_within_bar(3.5).round(1), 0.5, 0.05
  ...
  def test_time_until_subdivision_within_beat
    ...
    # beat not yet reached
    assert_in_delta @link.time_until_subdivision_within_beat(0.75).round(2), 0.25, 0.05
    # beat already passed - wait until the next bar
    assert_in_delta @link.time_until_subdivision_within_beat(0.25).round(2), 0.75, 0.05
  end
